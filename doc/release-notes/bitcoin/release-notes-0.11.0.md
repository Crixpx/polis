Polis Core version 0.11.0 is now available from:

  <https://polis.org/bin/polis-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer POLIS/kB fee to be
...
For example, add the following to `polis.conf`:
...
related to the blockchain in the polis system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Polis Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Polis Core will 
...
For an example script see [polis-submittx](https://github.com/laanwj/polis-submittx).
...
- #5334 `cf87536` libpolisconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new polis-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only PolisAmountField
- #5753 `0f44672` Add polis logo to about screen
...
- #5809 `46bfbe7` Add polis-cli man page
...
- #5849 `41259ca` contrib/init/polisd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Polis-Qt.app to Polis-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for polis-tx)
