...
The RPM spec file provided here is for Polis-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://polis.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Polis git repository](https://github.com/polispay/polis) but are not part of
...
Currently only `Source10` is used, for the example `polis.conf` file.
...
in the polis git repository. At present only an SVG version of the Polis
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the polis
...
needed when 0.12.1 is released, a proper fix is already in the Polis git
master branch.
...
idea to build Polis Core against a static version of that library compiled
...
The `polis.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `polis-server` package. At this point, most RPM based
...
The `polis-server` package should have SELinux support. How to properly do
...
Polis-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/polis/). He has been
packaging Polis for Fedora at least since 2012.
...
4. I renamed the `polis` package that contains the Qt GUI to `polis-core` as
that appears to be how the general population refers to it, in contrast to
`polis-xt` or `polis-classic`. I wanted to make sure the general population
