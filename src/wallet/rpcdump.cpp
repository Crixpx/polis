...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    return CPolisSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to polisd)\n"
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/polispay/polis/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by Polis %s\n", CLIENT_BUILD);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CPolisSecret(key).ToString(), strTime);
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see polisd log for details) and could "
