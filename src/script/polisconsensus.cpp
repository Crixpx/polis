...
int polisconsensus_verify_script_with_amount(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen, int64_t amount,
                                    const unsigned char *txTo        , unsigned int txToLen,
                                    unsigned int nIn, unsigned int flags, polisconsensus_error* err)
{
    CAmount am(amount);
                                   ...
                                   unsigned int nIn, unsigned int flags, polisconsensus_error* err)
{
    if (flags & polisconsensus_SCRIPT_FLAGS_VERIFY_WITNESS) {
        return set_error(err, polisconsensus_ERR_AMOUNT_REQUIRED);
    }
